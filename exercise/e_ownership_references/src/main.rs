...
fn inspect(my_str: &str) -> () {
    if my_str.ends_with("s") {
        println!("The string {} is plural!", my_str)
    } else {
        println!("The string {} not is singular!", my_str)
    }
}

fn change(in_str: &mut String) {
    if !in_str.ends_with("s") {
        in_str.push_str("s")
    }
}

fn eat(in_str: String) -> bool {
    in_str.starts_with("b") && in_str.contains("a")
}
